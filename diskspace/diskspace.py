...
parser.add_argument('directory', metavar='DIR', type=str, nargs='?', default='.', help='Directory to be analized')
parser.add_argument('-o', '--order', type=str, default='desc', choices=['desc', 'asc'],
                    help='The file order inside each folder')
parser.add_argument('-s', '--hide', type=int, default=0,
                    help='Hides all files that have a percentage lower than '                    'this value')
group = parser.add_mutually_exclusive_group()
group.add_argument('-a', '--all', help='Shows the full tree', action='store_true')
group.add_argument('-d', '--depth', help='Specifies the folder maximum depth to be analyzed', type=int, default=1)
parser.add_argument('-t', '--tree-view', action='store_true', help='Display the result in a tree mode')
...
def print_tree(file_tree, file_tree_node, path, largest_size, total_size, depth=0):
    ...
    print('{:>{}s} {:>4d}%  '.format(file_tree_node['print_size'], largest_size, percentage), end='')
    if args.tree_view:
    ...
    if len(file_tree_node['children']) != 0:
        for child in file_tree_node['children']:
            print_tree(file_tree, file_tree[child], child, largest_size, total_size, depth + 1)
...
def print_space_list(largest_size, file_tree, abs_directory, total_size):
    ...
    print_tree(file_tree, file_tree[abs_directory], abs_directory, largest_size, total_size)
...
def show_space_list(directory='.', depth=-1, order=True):
    ...
    for line in re.findall(line_regex, raw_output.strip(), re.MULTILINE):
        ...
        if file_path == abs_directory:
            ...
            if file_path in file_tree:
                ...
                file_tree[file_path] = {'children': [], 'size': file_size, }
        ...
        if dir_path not in file_tree:
            file_tree[dir_path] = {'children': [], 'size': 0, }
    ...
    for file_path in file_tree:
        ...
        file_tree_entry['children'] = sorted(file_tree_entry['children'], key=lambda v: file_tree[v]['size'],
                                             reverse=order
                                             )

        file_tree_entry['print_size'] = bytes_to_readable(file_tree_entry['size'])
